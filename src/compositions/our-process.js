...
import { Box, Center, Container, Flex, Heading, Stack, Text } from "@chakra-ui/react"
...
const ProcessBox = ({title, description, index}) => (
          <Stack w={{base: 'full', md: "32%"}} maxW="390px">
            ...
            <Flex align="center" color="violet">
              <Center zIndex={1} bg="white" boxSize="70px" mr="-35px" borderRadius="full" border="5px solid var(--chakra-colors-blue)" boxSizing="border-box">
                <Heading fontSize={24}>
                  {index+1}
                </Heading>
              </Center>
              <Box display="flex" alignItems="center" h="55px" pl="50px" flex={1} bg="none" bgImage="linear-gradient(90deg, white 0 75%, #ffffff00 100%)">
                <Heading fontSize={24}>{title}</Heading>
              </Box>
              {
                index < 2 && (
                  <Delayed onView={true} delay={0.4 * index}>
                  <ArrowForwardIcon fontSize={index === 0 ? 50 : 70} color="white" ml={-35}/>
                  </Delayed>
                )
              }
            </Flex>
...
const OurProcess = () => { 
  ...
  <Box w="full" pt={8} overflowX="clip">
    ...
    <Box w="full">
    <Container id="our-process" ref={containerRef} py={20} pb={{base: 40, md: 28}} maxW={1240} centerContent>
        <Heading
          textAlign="center"
          fontWeight={800}
        ...
        <Stack w="100%" mt={10} direction={{base: "column",md: "row"}} align={{base: "center", md: "start"}} justify="center" spacing={{base: 10, md: 3}}>
          {
            processList.map((obj, index) => (
              <ProcessBox key={"process-" + (index+1)} {...obj} index={index} />
            ))
          }
       </Stack> 
    </Container>

import { Box, Container, Image, Heading, SimpleGrid, Stack, Text} from '@chakra-ui/react'
...
const WorkExpectation = () => (
    ...
    <Container py="240px" maxW="930px">
      <Heading fontWeight={800} fontSize={['36px', '39px', '42px', '45px', '48px']}>
        WHAT
      ...
      <SimpleGrid mb={20} columns={{base: 2, md: 4}}>
        {
          ...
          <Delayed
            ...
            hoverable
          >
            <Stack mt={10} align="center" key={'expect-'+(i+1)} spacing={4}>
              <Image src={iconSrc} />
              <Box display="flex" h="65px" pt={1} alignItems="center" justifyContent="center">
                <Heading w="full" whiteSpace="pre-wrap" align="center" fontSize={{base: 18, md: 24}}>{title}</Heading>
              </Box>
              <Box w="full" align="center" px={5}>{description}</Box>
            </Stack>
          </Delayed>
          ))
        }
